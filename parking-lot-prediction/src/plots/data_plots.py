...
def occupancy_plot(df, path_saving_chart, days):
    """
    occupancy plot over a month from the start if days is not set.
    Otherwise it will show the percentage occupancy over the passed days.
    """
...
def charts(df, path_saving_chart, days_over_occupancy = 30):
    """
    days_over_occupancy is required if you want to visualize an interval over 30 days
    """
    occupancy_plot(df, path_saving_chart,days_over_occupancy)
