...
from utilities.metrics import true_pred_occ_parking
...
def one_step_ahead_subplot(ax, label, pred, truth, error, plot_error=True):
    ...
    intervall = max(truth) -  min(truth)
    ax.set_yticks(range(min(truth),max(truth)+int(intervall/5),int(intervall/5)))
    return ax
...
def multi_steps_ahead_subplot(ax, multiple_label, multiple_pred, multiple_truth, error, plot_error=True):
    ...
    intervall = max(multiple_truth[:, 0]) -  min(multiple_truth[:, 0])
    ax.set_xlim(left, right)
    #will set a grid for 5 values on the y axes
    ax.set_yticks(range(min(multiple_truth[:, 0]),max(multiple_truth[:, 0])+int(intervall/5),int(intervall/5)))
    return ax
