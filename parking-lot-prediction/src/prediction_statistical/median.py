...
def get_plot_and_print_accuracy(df: pd.DataFrame, path_plot: str, capacity: int, percentual_error: list):
    """
    Saves a madian plot to the given path and will print the accuracy to standard output.
    ...
    for per_error in percentual_error:
        ...
        accuracy.append((f"MEDIAN (±{per_error}%)",get_accuracy(df, error)))
        print('median accuracy [error=±{}(±{}%)] = {:.4f}'.format(error, per_error, accuracy[-1][1]))
