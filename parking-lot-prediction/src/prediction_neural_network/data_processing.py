...
def split_sequence(sequence, config_nn):
    """
    split a univariate sequence into samples and 
    returns an array of X containing sequance of n_timesteps_in hours and y containing the next
    n_timesteps_out hours after the first n_timesteps_in hours
    """
    X, y = list(), list()
...
def split_data(df, config_nn):
    """
    return the numpy array of the DataFrame splitted in testing and training set
    """
...
def add_weekday_hour(data):
    """
    This function apply to every sample adding the day of the week and the relative hour.
...
def get_data(X_train, y_train, X_test, y_test, config_nn):
    """
    Returns the sequence values converted in Tensors objects
    """
...
def process_data(df, config_nn,labels=False):
    """
    Process the DataFrame to obtain a tuple of X_train_data, X_test_data, y_train_data, y_test_data Tensor objects
    and another tuple of its relative timestamps
    """
