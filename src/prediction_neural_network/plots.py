...
from . import config
from .utilities import true_pred_occ_to_free_parking
...
def one_step_ahead_plot(label, pred, truth, path,hours_to_plot=None):
    ...
    plt.savefig(os.path.join(path,'1_step_prediction_NEW.svg'), bbox_inches='tight', format='svg')
    #plt.show()
...
def multi_steps_ahead_plot(label, pred, truth, path, hours_to_plot=None):
    ...
    plt.savefig(os.path.join(path,'8_step_prediction_NEW.svg'), bbox_inches='tight', format='svg')
